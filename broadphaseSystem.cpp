...
struct FillBuckets
{
	void operator()(std::vector<atomwrapper<int>>* bucketIndices, AABB* aabb, Position* pos, std::vector<BroadPhaseSystem::SpatialObject>* buckets)
	{
...
void BroadPhaseSystem::RunCollisionDetection(ecs::ECS* ecs)
{
	...
	std::cout << "reset buckets: " << ms << std::endl;
	...
	delta = end - begin;
	ms = std::chrono::duration_cast<std::chrono::nanoseconds>(delta).count() * 0.000001f;
	std::cout << "single thread fix indices: " << ms << std::endl;
	begin = std::chrono::high_resolution_clock::now();
#endif // PROFILEMS
	for (int i = 0; i < pos.comps.size(); i++)
	{
	...
	delta = end - begin;
	ms = std::chrono::duration_cast<std::chrono::nanoseconds>(delta).count() * 0.000001f;
	std::cout << "fill buckets: " << ms << std::endl;
	begin = std::chrono::high_resolution_clock::now();
#endif
